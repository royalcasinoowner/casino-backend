...
minesNamespace.on('connection', (socket) => {
    ...
    socket.on('clickTile', async ({ idx }, callback) => {
        ...
        try {
            const activeRound = await MinesEngine._getActiveRound(uid);
            if (!activeRound) return callback({ error: 'Game over' });
            
            const result = await MinesEngine.revealTile(uid, activeRound.roundId, idx);
            if (result.isMine) {
                // Game Over - lost
    ...
    socket.on('cashOut', async (callback) => {
        ...
        try {
            ...
            const result = await MinesEngine.cashout(uid, activeRound.roundId);
            socket.emit('betResult', { 
                win: true, 
